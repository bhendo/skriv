...
- **Foldable bundle unpacked**: `LivePreviewEditor` lists `defaultFoldableSyntaxExtensions`' members individually so upstream `bulletListExtension` (fixed "•") can be swapped for `ui/live-preview/bullets.ts` (depth-cycled • ◦ ▪, #84). On @prosemark/core upgrades, re-diff the bundle membership in `lib/fold/index.ts` as well as `prosemarkBasicSetup()` — and note the upstream export really is spelled `horizonalRuleExtension`.
- **Keyboard shortcuts:** all global chords live in the registry `ui/utils/shortcuts.ts` (#78) — keydown bindings, menu event names, and tooltip strings all derive from it, and `ui/__tests__/utils/shortcuts.test.ts` enforces parity with the menu.rs accelerators (menu.rs is read via Vite `?raw` import — `import.meta.url` is http-scheme under Vitest jsdom, so `node:fs` paths don't resolve from it). Source mode toggle is `Cmd+M`; Cmd+Shift+L is the outline three-state toggle. Cmd+E / Cmd+Alt+X are editor-internal aliases onto ProseMark's inline-code/strikethrough commands (`ui/live-preview/keymap.ts`), not registry entries.
- **Mouse handlers live on the content DOM**: CodeMirror registers its mouse/keyboard handlers on `view.contentDOM`, not `.cm-scroller` (and `eventBelongsToEditor` rejects events targeted outside it). Since #90 centers the prose column inside a full-pane scroller, margin clicks target the scroller and would be dead without `marginClickExtension` (`ui/utils/marginClick.ts`), which cancels the original mousedown and re-dispatches a clone on the content DOM — CM resolves position/click-count/modifiers from the event itself, so all gestures keep native semantics. Nothing in CM's input path checks `isTrusted`.
